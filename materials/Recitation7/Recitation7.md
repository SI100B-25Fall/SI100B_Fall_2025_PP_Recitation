...
- len()可以返回字符串的长度
- 可以通过s[index]来访问某一个字符
- 从前到后index从0开始，反之下标从-1开始
...
- 使用print()输出
- print(a, b, c)会使用空格分开
- 可以使用str()将别的类型转换成字符串输出，例：print("ans="+str(ans))
- print不换行：print("...", end="")
- temp = input("Input:")，打印提示文字并读取字符串到temp
- 使用int()等转换成需要的类型
- print(f'{x} factorial is {factorial}')在字符串中插入变量
<!--v--> 
...
## 迭代
- while循环：while(condition)...，设定正确的条件或者break避免一直循环
- break可以退出当前循环
- for循环：for variable in sequences of values: ...
- range(start, stop, step)：生成一个数字的序列
<!--v-->
...
- Tuple是一种不可变的类型
- 创建空的tuple：temp = ()
- 创建只有一个元素的tuple：temp = (1,)
- 同样支持slice等操作
- 可以用于交换：(x, y) = (y, x)
- 函数参数列表中使用*args依次将所有参数打包成一个元组
<!--v-->

## Lists
- List是一种可变(mutable)的类型
- 同样支持slice等操作
- 可以使用 + 将两个列表连接
- 可以直接使用index修改list中的某个值
- 对一个list使用append等函数可以修改这个list，例：L.append(5)
- s.split(' ')可以根据字符将string转换成list
- '_'.join(L)可以将list中每个元素用字符连接，转换成字符串
- 区分L.sort()，L.reverse()和sorted(L)
<!--v-->
...
print(L1, L2)
```
- 注意new_list = copy.copy(old_list)的用法
- 注意new_list = copy.deepcopy(old_list)的用法
<!--v-->
...
def make_prod(a):
	...
	return g
```
- 调用make_prod会返回一个函数对象
<!--v-->

## Exceptions, Assertions
- 使用try...except...else...finally...来处理异常
- 可以分别处理不同类型的异常，例：except TypeError
- finally是在异常处理之后总会被执行的语句
- Assert的用法：assert statement, "message"
<!--v-->

## Dictionaries
- 一个key对应一个value，key和value都是无序的
...
## Tips
- 写条件判断语句时，注意区分 = 和 ==
- 拷贝对象时，注意区分是clone还是alias
- 默认参数如果设置成可变，需要仔细处理
- 浮点数比较使用abs(a-b)<=1e-10的写法，1e-10是科学计数法表示1*10^(-10)
- 自定义函数时不要忘记return
- 写循环的时候注意不要写错列表的index、循环变量等
